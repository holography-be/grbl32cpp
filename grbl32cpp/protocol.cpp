...
#ifdef Report.ECHO_LINE_RECEIVED
	Report.echo_line_received(line);
#endif

	if (line[0] == 0) {
		// Empty or comment line. Send status message for syncing purposes.
		Report.status_message(STATUS_OK);
		...
		Report.status_message(STATUS_ALARM_LOCK);
...
void Cprotocol::main_loop()
{
	...
	if (sys.state == STATE_ALARM) {
		...
		Report.feedback_message(MESSAGE_ALARM_LOCK);
	}
	else {
		...
		System.execute_startup(line); // Execute startup script.
	}
			...
			if ((c == '\n') || (c == '\r')) { // End of line reached
				...
				if (comment != COMMENT_NONE) {
					...
					if (c <= ' ') {
						...
						Report.status_message(STATUS_OVERFLOW);
						comment = COMMENT_NONE;
						char_counter = 0;
					}
...
//		//		Report.alarm_message(ALARM_HARD_LIMIT_ERROR);
//		//	}
//		//	else if (rt_exec & EXEC_ALARM_SOFT_LIMIT) {
//		//		Report.alarm_message(ALARM_SOFT_LIMIT_ERROR);
//		//	}
//		//	else if (rt_exec & EXEC_ALARM_ABORT_CYCLE) {
//		//		Report.alarm_message(ALARM_ABORT_CYCLE);
//		//	}
//		//	else if (rt_exec & EXEC_ALARM_PROBE_FAIL) {
//		//		Report.alarm_message(ALARM_PROBE_FAIL);
//		//	}
//		//	else if (rt_exec & EXEC_ALARM_HOMING_FAIL) {
//		//		Report.alarm_message(ALARM_HOMING_FAIL);
//		//	}
//		//	// Halt everything upon a critical event flag. Currently hard and soft limits flag this.
//		//	if (rt_exec & EXEC_CRITICAL_EVENT) {
//		//		Report.feedback_message(MESSAGE_CRITICAL_EVENT);
...
//						Report.feedback_message(MESSAGE_SAFETY_DOOR_AJAR);
...
//							if (gc_state.modal.spindle != LASER_POWER_DISABLE) {
...
//							}
//							if (gc_state.modal.coolant != LASER_DISABLE) {
...
void Cprotocol::buffer_synchronize()
{
