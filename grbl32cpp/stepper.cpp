...
void Cstepper::st_wake_up()
{
	...
	if (bit_istrue(Settings.settings.flags, BITFLAG_INVERT_ST_ENABLE)) {
		...
		ENABLE_MOTOR_PORT->LATxSET.w = ENABLE_MOTOR_MASK;
		//STEPPERS_DISABLE_PORT &= ~(1 << STEPPERS_DISABLE_BIT); 
	}
	...
	if (sys.state & (STATE_CYCLE | STATE_HOMING)){
		...
		STEPPER_STEP_TIMER->tmxCon.set = TBCON_ON;
		//TIMSK1 |= (1 << OCIE1A);
	}
}
...
// Stepper shutdown
void Cstepper::st_go_idle()
{
	// Disable Stepper Driver Interrupt. Allow Stepper Port Reset Interrupt to finish, if active.
	STEPPER_STEP_TIMER->tmxCon.clr = TBCON_ON;
	clearIntEnable(STEPPER_STEP_IRQ);
...
void __USER_ISR irq_stepper_timer() {
	...
	DIR_PORT->LATxCLR.w = DIRECTION_MASK;	// Clear direction bits
	DIR_PORT->LATxSET.w = st.dir_outbits;
	...
	STEP_PORT->LATxSET.w = st.step_outbits;
	...
	if (st.exec_segment == NULL) {
		...
		if (segment_buffer_head != segment_buffer_tail) {
			...
			STEPPER_STEP_TIMER->tmxPr.reg = st.exec_segment->cycles_per_tick;
	...
	clearIntFlag(STEPPER_STEP_IRQ);
	if (Settings.settings.debug_mode == DEBUG_MODE_ON) { DEBUG_PORT->LATxCLR.w = DEBUG_MASK; }
}
...
void Cstepper::st_reset()
{
	...
	STEP_PORT->LATxCLR.w = STEP_MASK;
	DIR_PORT->LATxCLR.w = DIRECTION_MASK;
}
...
void Cstepper::init()
{
	...
	// Configure Timer : Stepper Driver Interrupt
	STEPPER_STEP_TIMER->tmxCon.reg = 0x0;	// prescaler 1:1
	STEPPER_STEP_TIMER->tmxTmr.reg = 0x0;
	STEPPER_STEP_TIMER->tmxPr.reg = 0x0;

	// Configure Timer : Stepper Port Reset Interrupt
	STEPPER_RESET_TIMER->tmxCon.reg = 0x0;	// prescaler 1:1
	STEPPER_RESET_TIMER->tmxTmr.reg = 0x0;
	STEPPER_RESET_TIMER->tmxPr.reg = 0x0;

	// set STEPPER interrupts
	setIntVector(STEPPER_STEP_VECTOR, irq_stepper_timer);
	setIntVector(STEPPER_RESET_VECTOR, irq_stepper_reset_timer);
	setIntPriority(STEPPER_STEP_IRQ, 3, 1);
	setIntPriority(STEPPER_RESET_IRQ, 4, 1);
	clearIntFlag(STEPPER_RESET_IRQ);
...
void Cstepper::st_update_plan_block_parameters()
{
...
float Cstepper::st_get_realtime_rate()
{
