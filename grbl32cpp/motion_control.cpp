...
void Cmotion::mc_line(float *target, float feed_rate, uint8_t invert_feed_rate, int32_t line_number)
#else
void Cmotion::mc_line(float *target, float feed_rate, uint8_t invert_feed_rate)
#endif
{
...
#ifdef USE_LINE_NUMBERS
void Cmotion::mc_arc(float *position, float *target, float *offset, float radius, float feed_rate,
...
void Cmotion::mc_dwell(float seconds)
{
	//if (sys.state == STATE_CHECK_MODE) { return; }
	//
...
#ifdef USE_LINE_NUMBERS
void Cmotion::mc_probe_cycle(float *target, float feed_rate, uint8_t invert_feed_rate, uint8_t is_probe_away,
